...
atomic update/uninstall и сохранность unrelated files. Полный native macOS/Linux/Windows lifecycle
E2E остаётся `REL-002` и не считается выполненным этим этапом.

## Этап 6 — Codex и Claude packages (частично завершён)
...
1. Добавить native clean-session install/discovery smoke evidence.
2. Прогнать clean-session behavioral/security evals.
3. Опубликовать compatibility matrix с подтверждёнными версиями clients.
...
Текущее состояние: adapters, fixed discovery roots, generated skill bundle, lifecycle engine и
policy guidance включены в `v0.4.0`. Repository tests подтверждают bytes, manifests и local
detection, но не заменяют clean-session Codex/Claude discovery и behavioral/security evals.
Поэтому `INT-011`, `INT-012` и `INT-014` остаются незакрытыми до сохранения такого evidence.

## Текущий цикл v0.5

Порядок по зависимостям:

1. Завершено: `DEV-004`, `DEV-005`, `DEV-006` и `DEV-012` предоставляют bounded Git identity,
   authenticated candidate search, trusted host mapping и отдельный untrusted repository context.
2. `DEV-014`: отделить repository-shared alias definitions от worktree-local active/recent state;
   хранить только bounded metadata с CAS, locking, retention и explicit erasure.
3. `DEV-001` и `DEV-002`, затем `DEV-003`/`DEV-013`: добавить bounded project, section,
   custom-field и task context, после чего реализовать central exact resolver без implicit selection.
4. `DEV-007`, затем `DEV-015`: добавить create task/subtask prepare/apply и immutable revisioned
   templates, полностью раскрываемые до approval.
5. `DEV-008`–`DEV-011`: membership, dependencies, attachment metadata и batch reads.
6. Закрыть `INT-011`, `INT-012`, `INT-014` и `REL-002`: сохранить clean-session client evidence
   и выполнить native integration lifecycle E2E до расширения на новые clients.
7. `DEV-016`, затем `INT-015`–`INT-018`: alias/template security evals перед Gemini, Copilot,
   OpenCode и Cursor expansion.
8. `REL-003`, `REL-004`, `REL-007`: provenance, SBOM и compatibility gates для следующего release.
