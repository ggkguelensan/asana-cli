...
|---|---|---|---|---|---|
| AP-001 | P0 | ready | Формализовать `agent_protocol_version` и `cli_version` в envelope | — | Версии присутствуют в capabilities и каждом agent response; есть compatibility test |
| AP-002 | P0 | ready | Расширить capability catalog метаданными action | AP-001 | Для каждой action заданы effect, approval class, input/output schema ID, limits и minimum CLI version |
| AP-003 | P0 | ready | Публиковать JSON Schema из runtime Zod-схем | AP-001 | `agent schema` и `agent schema ACTION` возвращают стабильный JSON; schema/runtime не расходятся в тестах |
...
| AP-006 | P0 | blocked | Спроектировать Zod-валидируемый durable operation record | AP-001 | Record содержит ID, action, target, immutable payload, guards, timestamps, TTL, state и protocol version |
| AP-007 | P0 | blocked | Реализовать atomic operation journal с restrictive permissions | AP-006 | Partial write не повреждает journal; чужие/невалидные records отклоняются; task/comment text не логируется отдельно |
| AP-008 | P0 | blocked | Перевести apply на `--operation-id` | AP-007 | Apply не принимает повторный payload; expired/stale/already-applied operations отклоняются стабильно |
...
| AP-012 | P1 | ready | Минимизировать `agent status` и общие projections | — | Email и другие необязательные PII отсутствуют по умолчанию |
| AP-013 | P1 | blocked | Добавить protocol compatibility/deprecation tests | AP-001, AP-003 | Старый совместимый клиент получает корректный ответ; несовместимый — machine-readable upgrade guidance |
