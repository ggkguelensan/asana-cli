...
- `agent context --git-current` для remote, repository, branch и commit;
- поиск по branch/commit/PR/issue/custom ID в имени, описании и разрешённых custom fields;
...
- a versioned repository context manifest with explicit workspace/project/section/custom-field/task mappings and revisioned structural task-create templates; no includes, interpolation, scripts, URLs or repository-defined authorization;
...
- bounded `agent context --alias` and `agent context --git-current` resolution: zero/multiple/truncated matches are returned as candidates and never selected implicitly;
...
- Homebrew, Scoop или эквивалентные каналы поставки;
- проверка совместимости CLI, skill bundle и agent protocol при update;
- macOS/Linux/Windows E2E с временным HOME и credential-store fixtures.
