...
Актуально на 2026-07-15. Текущая стабильная версия: `v0.2.0`.
...
- агент может найти связанную с текущей веткой/коммитом задачу, прочитать нужный контекст,
  подготовить безопасное изменение и применить его после approval;
- обычный developer workflow не требует `api call` или `request`;
- writes по-прежнему ограничены policy, одной операцией и явным apply.
