...
2. `DEV-006` is complete: a host-administered fixed-path, strict exact repository-to-Asana mapping is exposed only through local read-only `agent context --repository-asana`; it remains advisory context and cannot affect write authorization. `DEV-012` is also complete: its separate local/no-PAT/no-network `agent context --repository-context` reads only the untrusted fixed-root `.asana-cli/repository-context.json` strict bounded manifest, reports deterministic revision/digest and exact fully qualified immutable-GID aliases, and has no merge/precedence, resolver, candidate handoff, or write effect. DEV-013 remains the resolver, DEV-014 the alias lifecycle/state, and DEV-015 templates. DEV-006 completion records targeted contract/security evidence only; it does not claim native Windows runtime E2E from this Darwin development environment.
