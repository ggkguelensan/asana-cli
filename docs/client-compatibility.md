...
A client is `supported` only when a passed behavioral/security evidence record exists. Qualification applies to the primary `asana` skill; onboarding-suite behavior is covered by the current deterministic lifecycle and CLI tests. A deterministic install adapter without that record remains `experimental`; the shared portable format remains `generic`.
