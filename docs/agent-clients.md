...
- Gemini CLI: `.gemini/skills/asana`
...
## Client-specific packaging and permissions

Gemini CLI also has a deterministic extension package in
`generated/gemini-extension`. Its manifest contains no `mcpServers`, settings, hooks, credentials,
or executable code; `skills/asana` is generated byte-for-byte from the canonical skill. The
managed `.gemini/skills/asana` path remains the direct user/project install route.

GitHub Copilot CLI uses the native skill directories above. The canonical `SKILL.md` deliberately
omits `allowed-tools`; do not add `allowed-tools: shell` or `allowed-tools: bash`, because those
forms remove the per-command confirmation boundary.

For OpenCode, keep skill loading separate from shell execution. A narrow starting point is:
...
```json
{
  ...
  "permission": {
    ...
    "bash": {
      "*": "ask",
      "asana-cli api *": "deny",
      "asana-cli request *": "deny",
      "asana-cli auth *": "deny",
      "asana-cli integrations *": "deny",
      "asana-cli agent apply *": "ask"
    }
  }
}
```

Do not run OpenCode with `--auto` for this workflow: that mode approves rules which would
otherwise ask. Add only reviewed read/prepare prefixes from `integrations policy opencode`; keep
every apply approval-required.

Cursor discovers the same canonical skill through its native `.cursor/skills` root. Cursor shell
permissions are coarse, so leave shell confirmation enabled and approve the exact
`asana-cli agent apply --operation-id UUID` separately after reviewing the prepared operation.

Native paths and permission behavior are pinned to the vendors' current documentation:
...
Use `asana-cli integrations policy CLIENT` to print display-only guidance for any registry
